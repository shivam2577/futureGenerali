		...
		if(isset($_POST["username"]) && isset($_POST["phno"]) && isset($_POST["confirmpass"]) && isset($_POST["password"]) && isset($_POST["email"])&& isset($_POST["slocation"])&& isset($_POST["sbranch"]))
		{
			...
			if($_SERVER["REQUEST_METHOD"]=="POST")
			{
				if(!(empty($name) || empty($email) || empty($contact) || empty($password) || empty($pass_confirm) ))
				{
					...
					{
						$query = "SELECT tcontact FROM tteam WHERE tcontact = '{$contact}'";
					...
					if($f == 1){
						$query = "INSERT INTO tteam(tname,tcontact,temail,tpassword,tlocation,tbranch,tvkey) VALUES('$name','$contact','$email','$password','$location','$branch','$vkey');";
						...
						if($result&&$result1){
							...
							$to=$bemail;
						    $subject="Please verify the new trainer";
						    $message="Dear $bname,<br>The details of the trainer in your branch are:<br>Trainer Name: $name<br>Trainer e-mail: $email<br>Trainer Contact No: $contact<br>Click <a href='http://localhost/future_agents/trainsignup/verify.php?vkey=$vkey'>here</a> to verify their account.";
